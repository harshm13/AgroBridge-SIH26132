...
import { Store, ShieldCheck, MapPin, Truck, Scale, ChevronRight, CheckCircle2, AlertCircle, Info, Calculator, IndianRupee, Sparkles } from 'lucide-react';
...
export default function Marketplace() {
    ...
    return (
        <div className="ab-container py-6 sm:py-8 space-y-6">
            ...
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm min-h-[500px]">
                                ...
                                return (
                                    <div key={buyer.id} className={`ab-offer-card relative ${isBest ? 'ring-2 ring-emerald-500 shadow-lg' : ''}`}>
                                        ...
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2 cursor-pointer hover:text-emerald-700" onClick={() => setSelectedBuyer(buyer)}>
                                                    {buyer.name}
                                        ...
                                        <button
                                            onClick={() => setContractBuyer(buyer)}
                                            className="w-full ab-primary-btn !py-3 flex items-center justify-center gap-2 cursor-pointer active:scale-95 transition"
                                        >
                ...
                {activeTab === 'compare' && (
                    <div className="p-4 sm:p-6">
                        <div className="ab-comparison-wrapper border border-slate-200 shadow-sm">
                            <table className="ab-comparison-table">
                                <thead>
                                    <tr>
                                        <th className="text-slate-600 font-extrabold uppercase text-xs tracking-wider">Metric Parameter</th>
                                        <th className="best-col relative">
                                            <div className="inline-flex items-center gap-1 bg-emerald-600 text-white text-[10px] font-black px-2.5 py-0.5 rounded-full mb-2 uppercase tracking-widest shadow-sm">
                                                <Sparkles className="w-3 h-3" /> AI Recommendation
                                            </div>
                                            <div className="text-base font-black text-slate-900">{sortedBuyers[0]?.name || 'FreshMart Inc.'}</div>
                                            <div className="text-xs font-bold text-slate-500 mt-0.5">{sortedBuyers[0]?.type || 'Retail Chain'}</div>
                                        </th>
                                        <th>
                                            <div className="text-base font-black text-slate-900">{sortedBuyers[1]?.name || 'Nashik APMC'}</div>
                                            <div className="text-xs font-bold text-slate-500 mt-0.5">{sortedBuyers[1]?.type || 'Mandi'}</div>
                                        </th>
                                        <th>
                                            <div className="text-base font-black text-slate-900">{sortedBuyers[2]?.name || 'AgriExport Ltd'}</div>
                                            <div className="text-xs font-bold text-slate-500 mt-0.5">{sortedBuyers[2]?.type || 'Exporter'}</div>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td className="font-bold text-slate-700">Gross Offered Price</td>
                                        <td className="font-black text-slate-900 text-base best-col">₹{sortedBuyers[0]?.price} / Qtl</td>
                                        <td className="font-bold text-slate-700 text-base">₹{sortedBuyers[1]?.price} / Qtl</td>
                                        <td className="font-black text-emerald-600 text-base">₹{sortedBuyers[2]?.price} / Qtl</td>
                                    </tr>
                                    <tr>
                                        <td className="font-bold text-slate-700">Transport Cost (Est)</td>
                                        <td className="text-amber-700 font-bold best-col">-₹{sortedBuyers[0]?.transportCostPerQtl} / Qtl</td>
                                        <td className="text-amber-700 font-bold">-₹{sortedBuyers[1]?.transportCostPerQtl} / Qtl</td>
                                        <td className="text-red-500 font-bold">-₹{sortedBuyers[2]?.transportCostPerQtl} / Qtl</td>
                                    </tr>
                                    <tr className="net-row">
                                        <td className="font-black text-emerald-900 text-base">Net Price (In Hand)</td>
                                        <td className="font-black text-emerald-700 text-xl best-col">
                                            ₹{sortedBuyers[0] ? sortedBuyers[0].price - sortedBuyers[0].transportCostPerQtl : 3075} / Qtl
                                        </td>
                                        <td className="font-bold text-slate-800 text-lg">
                                            ₹{sortedBuyers[1] ? sortedBuyers[1].price - sortedBuyers[1].transportCostPerQtl : 2575} / Qtl
                                        </td>
                                        <td className="font-bold text-slate-900 text-lg">
                                            ₹{sortedBuyers[2] ? sortedBuyers[2].price - sortedBuyers[2].transportCostPerQtl : 3190} / Qtl
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="font-bold text-slate-700">Quality Required</td>
                                        <td className="best-col">
                                            <span className="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-lg text-xs font-black">
                                                {sortedBuyers[0]?.needs || 'Grade A'}
                                            </span>
                                        </td>
                                        <td>
                                            <span className="inline-block bg-slate-100 text-slate-700 px-3 py-1 rounded-lg text-xs font-black">
                                                {sortedBuyers[1]?.needs || 'Any'}
                                            </span>
                                        </td>
                                        <td>
                                            <span className="inline-block bg-purple-100 text-purple-800 px-3 py-1 rounded-lg text-xs font-black">
                                                {sortedBuyers[2]?.needs || 'Zero Residue'}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="font-bold text-slate-700">Platform Trust Score</td>
                                        <td className="best-col">
                                            <div className="flex items-center gap-1.5 font-black text-emerald-700">
                                                <ShieldCheck className="w-5 h-5 text-emerald-600" />
                                                <span>{sortedBuyers[0]?.trust || 98}/100</span>
                                            </div>
                                        </td>
                                        <td>
                                            <div className="flex items-center gap-1.5 font-bold text-amber-700">
                                                <ShieldCheck className="w-5 h-5 text-amber-500" />
                                                <span>{sortedBuyers[1]?.trust || 85}/100</span>
                                            </div>
                                        </td>
                                        <td>
                                            <div className="flex items-center gap-1.5 font-black text-emerald-700">
                                                <ShieldCheck className="w-5 h-5 text-emerald-600" />
                                                <span>{sortedBuyers[2]?.trust || 92}/100</span>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td className="font-bold text-slate-700">Payment Terms</td>
                                        <td className="font-bold text-slate-800 best-col">{sortedBuyers[0]?.payment}</td>
                                        <td className="font-bold text-slate-800">{sortedBuyers[1]?.payment}</td>
                                        <td className="font-bold text-slate-800">{sortedBuyers[2]?.payment}</td>
                                    </tr>
                                    <tr>
                                        <td className="font-bold text-slate-700">Action</td>
                                        <td className="best-col">
                                            <button
                                                onClick={() => setContractBuyer(sortedBuyers[0])}
                                                className="ab-primary-btn w-full !py-2.5 text-sm shadow-md shadow-emerald-200"
                                            >
                                                Accept Best Offer
                                            </button>
                                        </td>
                                        <td>
                                            <button
                                                onClick={() => setContractBuyer(sortedBuyers[1])}
                                                className="ab-secondary-btn w-full !py-2.5 text-sm"
                                            >
                                                Accept Offer
                                            </button>
                                        </td>
                                        <td>
                                            <button
                                                onClick={() => setContractBuyer(sortedBuyers[2])}
                                                className="ab-secondary-btn w-full !py-2.5 text-sm"
                                            >
                                                Accept Offer
                                            </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            {selectedBuyer ? (
                                <div className="ab-realization p-6 md:p-8">
                                    ...
                                    <button
                                        onClick={() => setContractBuyer(selectedBuyer)}
                                        className="w-full ab-primary-btn !py-4 mt-8 text-lg shadow-lg shadow-emerald-200 cursor-pointer active:scale-95 transition"
                                    >
                                        Proceed to Contract
                                    </button>
                                </div>
            ...
            {contractBuyer && (
                <ContractModal offerData={contractBuyer} onClose={() => setContractBuyer(null)} />
            )}
        </div>
    );
}
